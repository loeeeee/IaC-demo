...

{
  ...
  networking.hostName = "tuff"; # Define your hostname.
  ...
  services.openssh = {
    enable = true;
    openFirewall = false; # REQUIRE MANUALLY FIREWALL RULES!
    settings = {
  ...
  # Disable NixOS firewall - we use nftables instead
  networking.firewall = {
    enable = false;
  };
  
  # Enable nftables with custom ruleset
  networking.nftables = {
    enable = true;
    flushRuleset = true;
    rulesetFile = ./nftables.nft;
    checkRuleset = false; # Skipped because wg does not exist on build
  };

  # Ensure nftables starts after WireGuard interface is up
